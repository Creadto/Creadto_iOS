...
@objc public protocol BackCameraManagerDelegate: AnyObject {
    func cameraDidOutput(colorBuffer: CVPixelBuffer, depthBuffer: CVPixelBuffer, depthCalibrationData: AVCameraCalibrationData)
    @objc optional func cameraManagerDidStartSession(_ manager: BackCameraManager)
    @objc optional func cameraManagerDidNotStartSession(_ manager: BackCameraManager, result: BackCameraManager.SessionSetupResult)
}

public extension Notification.Name {
    /** Analogous to CameraManagerDelegate.cameraManagerDidStartSession */
    static let BackCameraManagerDidStartSession = NSNotification.Name("CameraManagerDidStartSession")
}
 ...
 */
@objc public class BackCameraManager: NSObject, AVCaptureDataOutputSynchronizerDelegate {
    ...
    @objc public weak var delegate: BackCameraManagerDelegate!
    ...
    @objc public func startSession(_ completion: ((SessionSetupResult) -> Void)? = nil) {
        guard BackCameraManager.isDepthCameraAvailable else {
    ...
    private let _videoDeviceDiscoverySession = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInLiDARDepthCamera],
                                                                                mediaType: .video,
                                                                                position: .back)
