...
//@objc public protocol BackScanningViewControllerDelegate: AnyObject {
//    func scanningViewControllerDidCancel(_ controller: BackScanningViewController)
//    @objc optional func scanningViewController(_ controller: BackScanningViewController, didScan pointCloud: SCPointCloud)
//}
//
//extension BackScanningViewController: BackScanningViewControllerDelegate {
//    func scanningViewControllerDidCancel(_ controller: BackScanningViewController) {
...
//    }
//
//    func scanningViewController(_ controller: BackScanningViewController, didScan pointCloud: SCPointCloud) {
 ...
 */
@objc open class BackScanningViewController: UIViewController,
    BackCameraManagerDelegate,
    SCReconstructionManagerDelegate
{
    ...
    //@objc public weak var delegate: BackScanningViewControllerDelegate?
    ...
    @objc public func stopScanning(reason: ScanningTerminationReason) {
        ...
        if reason == .finished {
            ...
            _reconstructionManager.finalize {
                ...
                let vc = ScenePreviewViewController(pointCloud: pointCloud, meshTexturing: self.meshTexturing, landmarks: nil)
                vc.leftButton.addTarget(self, action: #selector(self.dismissPreviewedScanTapped), for: UIControl.Event.touchUpInside)
                vc.rightButton.addTarget(self, action: #selector(self.savePreviewedSceneTapped), for: UIControl.Event.touchUpInside)
                vc.leftButton.setTitle("Rescan", for: UIControl.State.normal)
                vc.rightButton.setTitle("Save", for: UIControl.State.normal)
                self.scenePreviewVC = vc
    ...
    @objc public var generatesTexturedMeshes: Bool = true {
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        guard BackCameraManager.isDepthCameraAvailable else { return }
    ...
    override open func viewWillDisappear(_ animated: Bool) {
        ...
        stopScanning(reason: BackScanningViewController.ScanningTerminationReason.canceled)
    ...
    private let _cameraManager = BackCameraManager()
